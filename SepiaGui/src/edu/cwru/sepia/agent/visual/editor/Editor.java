...
public class Editor extends JFrame {
	...
	public Editor(GameScreen screen, GamePanel gamePanel, State state, String templatefilename) {
		...
		addPlayer.addActionListener(new ActionListener() {
			...
			State state;
			public ActionListener setOutsideInformationAndClick(DefaultComboBoxModel<String> model, String templatefilename, State state) {
				this.model = model;
	...
	public static void main(String[] args) throws FileNotFoundException, JAXBException {
		...
		for (int argInd = 0; argInd < args.length; argInd++)
		{
			if (args[argInd].equals("--loadstate"))
			{
				...
				argInd++;
				try {
				state = SerializerUtil.loadState(args[argInd]);
				}
				catch(Exception e) {
