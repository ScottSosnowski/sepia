...
public class GamePanel extends JPanel {
	...
	public static final Color[] playerColors = new Color[] { new Color(255, 0, 0),
			new Color(0, 255, 0), new Color(0, 0, 255), new Color(255, 255, 0),
			new Color(255, 0, 255), new Color(0, 255, 255), new Color(255, 255, 255),
			new Color(0, 0, 0) };
	...
	public void paintComponent(Graphics g) {
		...
		g.fillRect(scaleX(0), scaleY(0), scaleX(currentState.getXExtent()) - scaleX(0),
				scaleY(currentState.getYExtent()) - scaleY(0));// background
		...
		if(latestHistory != null) {
			...
			for(RevealedResourceNodeLog rrl : latestHistory.getRevealedResourceNodeLogs()) {
				...
				String typeName = rrl.getResourceNodeType().getName();
				if(typeName.equals("GOLD_MINE")) {
					revealedMine.draw(g, x, y);
				} else if(typeName.equals("TREE")) {
					revealedTree.draw(g, x, y);
				} else {
					DrawingStrategy.charGraphic(typeName.toUpperCase().charAt(0)).draw(g, x, y);
				}
			}
		}
		// draw trees
		DrawingStrategy tree = DrawingStrategy.treeGraphic();
		for(int id : currentState.getResourceNodeIds("TREE")) {
		...
		for(int id : currentState.getResourceNodeIds("GOLD_MINE")) {
			ResourceView node = currentState.getResourceNode(id);
			int x = scaleX(node.getXPosition());
			int y = scaleY(node.getYPosition());
			if(x < 0 || y < 0)
		...
		for(int id : currentState.getAllUnitIds()) {
			...
			if(x < 0 || y < 0 || x > getWidth() || y > getHeight())
				continue;
			DrawingStrategy letter = DrawingStrategy.charGraphic(unit.getTemplateView()
					.getCharacter());
		...
		if(latestHistory != null) {
			...
			for(DamageLog damage : latestHistory.getDamageLogs(currentState.getTurnNumber() - 1)) {
				...
				if(attacker != null && defender != null) {
					// do an offset so you can see two things shooting at each
					// other
					int yplayeroffset = damage.getAttackerController() * 2;
					g.drawLine(scaleX(attacker.getXPosition()) + SCALING_FACTOR / 2,
							scaleY(attacker.getYPosition()) + SCALING_FACTOR / 2,
							scaleX(defender.getXPosition()) + SCALING_FACTOR / 2,
							scaleY(defender.getYPosition()) + SCALING_FACTOR / 2 + yplayeroffset);
					g.drawString(
							Integer.toString(damage.getDamage()),
							(int) (scaleX(attacker.getXPosition()) * 0.75 + scaleX(defender
									.getXPosition()) * 0.25) + SCALING_FACTOR / 2,
							(int) (scaleY(attacker.getYPosition()) * .75 + scaleY(defender
									.getYPosition()) * .25) + SCALING_FACTOR / 2 + yplayeroffset);
				} else if(attacker != null) {
					// Just draw the number over the attacker
					g.drawString(Integer.toString(damage.getDamage()),
							scaleX(attacker.getXPosition()) + SCALING_FACTOR / 2,
							scaleY(attacker.getYPosition()) + SCALING_FACTOR / 2);
				} else if(defender != null) {
					// Just draw the number over the defender
					g.drawString(Integer.toString(damage.getDamage()),
							scaleX(defender.getXPosition()) + SCALING_FACTOR / 2,
							scaleY(defender.getYPosition()) + SCALING_FACTOR / 2);
				}
		...
		g.drawString(currentState.getXExtent() + "x" + currentState.getYExtent(), getWidth() - 50,
				getHeight() - 1);
