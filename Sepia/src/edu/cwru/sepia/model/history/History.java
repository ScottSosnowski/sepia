...
public class History implements DeepEquatable {
	...
	public void recordUpgrade(UpgradeTemplate upgradetemplate, Unit creator, State state) {
		...
		for(PlayerHistory playerHistory : playerHistories.values())
		{
			if (state.canSee(x, y, playerHistory.playerNumber))
			{
				playerHistory.getEventLogger().recordUpgrade(state.getTurnNumber(), upgradetemplate.getID(), creator.id, upgradetemplate.getPlayer());
			}
		}
		observerHistory.getEventLogger().recordUpgrade(state.getTurnNumber(), upgradetemplate.getID(), creator.id, upgradetemplate.getPlayer());		
	}
