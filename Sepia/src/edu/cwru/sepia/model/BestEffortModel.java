...
public class BestEffortModel extends AbstractDurativeModel {
	...
	public BestEffortModel(State init, StateCreator restartTactic, Configuration configuration) {
		super(init, restartTactic, configuration, Logger.getLogger(BestEffortModel.class
				.getCanonicalName()));
				...
				while(queuedActItr.hasNext()) {
					...
					do {
						...
						case PRIMITIVEMOVE:
							...
							if(state.inBounds(xPrime, yPrime) && u.canMove()
									&& empty(u, xPrime, yPrime)) {
								...
								boolean willcompletethisturn = newdurativeamount == DurativePlanner
										.calculateMoveDuration(u, u.getXPosition(),
												u.getYPosition(), d);
								...
								failedTry = true;
								queuedAct.resetPrimitives(calculatePrimitives(queuedAct
										.getFullAction()));
							}
							break;
						case PRIMITIVEGATHER:
							...
							if(resource == null) {
								...
								// action
								if(willcompletethisturn) {
									int amountPickedUp = resource.reduceAmountRemaining(u
											.getTemplate().getGatherRate(
						...
						case PRIMITIVEBUILD: {
							...
							if(queuedAct.getFullAction().getType() == ActionType.COMPOUNDBUILD
								...
								LocatedProductionAction fullbuild = (LocatedProductionAction) queuedAct
										.getFullAction();
								if(fullbuild.getX() != u.getXPosition()
										|| fullbuild.getY() != u.getYPosition()) {
									failedTry = true;
									queuedAct.resetPrimitives(calculatePrimitives(queuedAct
											.getFullAction()));
									break;
							...
							if(u.getTemplate().canProduce(template)) {
								...
								if(prerequisitesMet) {
									int newdurativeamount = a
											.equals(u.getActionProgressPrimitive()) ? u
											.getActionProgressAmount() + 1 : 1;
									boolean willcompletethisturn = newdurativeamount == DurativePlanner
						...
						case PRIMITIVEPRODUCE: {
							...
							Template<?> template = state.getTemplate(((ProductionAction) a)
									.getTemplateId());
								...
								if(prerequisitesMet) {
									...
									if(willcompletethisturn) {
										if(template instanceof UnitTemplate) {
											Unit produced = ((UnitTemplate) template)
													.produceInstance(state);
											int[] newxy = state.getClosestPosition(x, y);
											...
											if(state.tryProduceUpgrade(upgradetemplate
													.produceInstance(state))) {
						...
						case FAILED: {
							failedTry = true;
							queuedAct
									.resetPrimitives(calculatePrimitives(queuedAct.getFullAction()));
							break;
						}
						...
						} else if(a.getType() == ActionType.FAILEDPERMANENTLY || failedTry
								&& fullIsPrimitive) {
