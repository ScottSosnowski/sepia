...
public abstract class AbstractModel implements Model {
	...
	protected boolean accessible(Unit unit, int x, int y) {
		return 
				state.inBounds(x, y) //Not out of the area
				&& unit.getTemplate().canMove() && unit.getTemplate().getDurationMove(state.terrainAt(x, y)) > 0 //The unit is able to move on the specified terrain
				&& (state.unitAt(x, y) == null || state.unitAt(x, y).equals(unit)) && state.resourceAt(x, y) == null;//No other unit or resource is on the square
	}
	/**
	 * Accessible for an unmade unit of a specific template
	 * @param unitTemplate
	 * @param x
	 * @param y
	 * @return
	...
	protected boolean accessible(UnitTemplate unitTemplate, int x, int y) {
		return 
				state.inBounds(x, y) //Not out of the area
				&& unitTemplate.canMove() && unitTemplate.getDurationMove(state.terrainAt(x, y)) > 0 //The unit is able to move on the specified terrain
				&& state.unitAt(x, y) == null && state.resourceAt(x, y) == null;//No other unit or resource is on the square
	}
