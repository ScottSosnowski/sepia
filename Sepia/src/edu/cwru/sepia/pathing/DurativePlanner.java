...
/**
 * An implementation of basic planning methods extended to the case of actions
 * that take time to act.
 */
public class DurativePlanner implements Serializable {
	 ...
	 * 
	 * @param u
	 *            The unit being moved. Necessary for calculating durative
	 *            actions
	 * @param startingx
	 * @param startingy
	 * @param endingx
	 ...
	 * @param isFirstMove
	 *            Whether this is the first move in a larger plan (and thus that
	 *            the current progress toward a move should be included)
	 * @return A move plan with the success, path, and final x and y
	 *         coordinates.
	 */
	public MovePlan getDirections(Unit u, int startingx, int startingy, int endingx, int endingy,
			int tolerancedistance, boolean isFirstMove) {
		// System.out.println("calling A* to get the unit at "+u.getxPosition()
		// + ","+u.getyPosition() + " from "+startingx + "," +
		// startingy+" to "+endingx+","+endingy +
		// " within a tolerance of "+tolerancedistance);
		...
		while(!queue.isEmpty() && bestvalidpath == null) {
			...
			int chebydisttogoal = currentplace.asRectangle().distanceTo(new Rectangle(endingx, endingy));
			...
			} else // not done
			{
				...
				for(Direction d : Direction.values()) {
					int newx = currentplace.getX() + d.xComponent();
					int newy = currentplace.getY() + d.yComponent();

					int newdisttogoal = Math.max(Math.abs(newx - endingx), Math.abs(newy - endingy));
					// valid if the new state is within max distance and
					// unoccupied
					if(state.inBounds(newx, newy)
						...
						int durativestep = calculateMoveDuration(u, currentplace.getX(), currentplace.getY(), d);
						// only proceed if the durative step is nonnegative and
						// less than the maximum, otherwise assume impassibility
						if(durativestep >= 1 && durativestep <= calculateMaxMoveDuration(u)) {
							// If this is the first move, then you can continue
							...
							AStarNode newnode = new AStarNode(newx, newy, durativedisttogoal, currentplace, d,
									durativestep);
		...
		if(bestvalidpath == null)
			...
			int lastx = bestvalidpath.getX();
			int lasty = bestvalidpath.getY();
			while(bestvalidpath.getPrevious() != null) {
				// always send false for whether to continue an existing action,
				// as that is included in durativesteps
				doDuratives(toreturn, u, Action.createPrimitiveMove(u.id, bestvalidpath.getDirectionFromPrevious()),
						calculateMaxMoveDuration(u), false, true);
				bestvalidpath = bestvalidpath.getPrevious();
			}
	...
	// (state.unitAt(endingx, endingy)!=null || state.resourceAt(endingx,
	// endingy)!=null);
	// AStarNode bestnode= null;
	// queue.offer(new
	// AStarNode(startingx,startingy,Math.max(Math.abs(startingx-endingx),
	// Math.abs(startingy-endingy))));
	...
	// DistanceMetrics.chebyshevDistance(currentnode.x, currentnode.y, endingx,
	// endingy);
	// //Check if you are done
	// if (tolerancedistance >= currentdistancetogoal || (!cancollideonfinal &&
	// currentdistancetogoal == 1 &&collidesatend))
	// {
	...
	//
	// int newdisttogoal =
	// Math.max(Math.abs(newx-endingx),Math.abs(newy-endingy));
	// //valid if the new state is within max distance and is in bounds and
	// either there is no collision or it is at the target
	// if (state.inBounds(newx, newy) &&
	// (
	// (state.unitAt(newx, newy) ==null && state.resourceAt(newx, newy)==null)
	// ||
	...
	// )
	// {
	// AStarNode newnode = new AStarNode(newx, newy, currentnode.g+1,
	// currentnode.g+1+newdisttogoal, currentnode, d);
	// if(!checked.contains(newnode))
	// {
	// queue.offer(newnode);
	// checked.add(newnode);
	...
	public LinkedList<Action> planAction(Unit actor, Action action) {
		switch(action.getType()) {
			...
			case FAILED:
				// The only primitive action needed to execute a primitive
				// action is itself
	...
	public LinkedList<Action> planMove(Unit actor, int x, int y) {
		...
		if(!plan.succeeded) {// No path to the unit
	...
	public LinkedList<Action> planAttack(Unit actor, Unit target) {
		...
		// plan a route to onto the resource
		MovePlan plan = getDirections(actor, actor.getXPosition(), actor.getYPosition(), target.getXPosition(),
				target.getYPosition(), actor.getTemplate().getRange(), true);
		if(!plan.succeeded) {// No path to the unit
			return planFail(actor.id);
		}
	...
	/**
	 * Plan a compound deposit action in which the unit moves next to the target
	 * town hall and then deposits into it
	 * 
	 * @param actor
	 * @param target
	 * @param distance
	...
	public LinkedList<Action> planDeposit(Unit actor, Unit target) {
		...
		// plan a route to onto the resource
		MovePlan plan = getDirections(actor, actor.getXPosition(), actor.getYPosition(), target.getXPosition(),
				target.getYPosition(), 1, true);
		if(!plan.succeeded) {// No path to the unit
			return planFail(actor.id);
		}
		boolean noPreviousProgress = plan.actions.isEmpty();
		Action primitivetorepeat = Action.createPrimitiveDeposit(actor.id,
				Direction.getDirection(target.getXPosition() - plan.finalx, target.getYPosition() - plan.finaly));
	...
	/**
	 * Plan a compound gather action in which the unit moves next to the target
	 * node and then gathers from it.
	 * 
	 * @param actor
	 * @param target
	...
	public LinkedList<Action> planGather(Unit actor, ResourceNode target) {
		...
		Action primitivetorepeat = Action.createPrimitiveGather(actor.id,
				Direction.getDirection(xDirection, yDirection));
	...
	public LinkedList<Action> planBuild(Unit actor, int targetX, int targetY, UnitTemplate template) {
		...
		boolean noPreviousProgress = plan.actions.isEmpty();
		// needs to know how much building on the target template the unit
		// already has done
	...
	/**
	 * Add a sequence of primitive actions to a list. This can add to either the
	 * front or the back of the list and is capable of adding only enough to
	 * complete a partially completed action.
	 * 
	 * @param listToAddTo
	 *            The list of actions. This list will have primitives added to
	 *            it's tail.
	 * @param unit
	 *            The unit that will do the action.
	 * @param primitive
	 *            The primitive action which will be repeated until done.
	 * @param duration
	 *            Full duration of the action in question.
	 * @param immediatelyNext
	 *            Whether the action is immediately next, and thus whether the
	 *            unit's current status should be considered.
	 * @param addToFront
	 *            Whether to add the new actions to the front or back
	 */
	private void doDuratives(LinkedList<Action> listToAddTo, Unit unit, Action primitive, final int duration,
			final boolean immediatelyNext, final boolean addToFront) {
	 ...
	 * 
	 * @param u
	 *            The unit whose maximum move we are probing.
	 * @return
	 */
