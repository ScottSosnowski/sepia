...
public class DurativePlanner implements Serializable {
	...
	public MovePlan getDirections(Unit u, int startingx, int startingy, int endingx, int endingy,
		...
		//The heuristic needs a basis to calculate estimated distances to the goal that are never pessimistic
		int minimumMoveDuration = calculateMinimumMoveDuration(u);
			...
			} else // not done
			{
				...
				for(Direction d : Direction.values()) {
					...
					if(state.inBounds(newx, newy)
						...
						int durativedisttogoal = minimumMoveDuration*newChebyshevDistToGoal;
						int durativestep = calculateMoveDuration(u, currentplace.getX(), currentplace.getY(), d, state);
		...
		if(bestvalidpath == null)
			...
			while(bestvalidpath.getPrevious() != null) {
				...
				doDuratives(toreturn, u, Action.createPrimitiveMove(u.id, bestvalidpath.getDirectionFromPrevious()),
						bestvalidpath.getDurativeSteps(), false, true);
