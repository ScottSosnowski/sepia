...
public class BasicGatheringCoordinator implements Serializable {
	 ...
	 * @param actions
	 */
	public void assignActions(StateView state, RelevantStateView relstate, Collection<Action> actions) {
		if (verbose)
		{
		...
		for (Integer minerID : miners)
		{
			...
			else
			{
				...
				if (closestID != Integer.MIN_VALUE) {
					...
					actions.add(Action.createCompoundGather(minerID, closestID));
				}
				else {
					if (verbose) {
		...
		for (Integer lumberjackID : lumberjacks)
		{
			...
			else
			{
				...
				if (closestID != Integer.MIN_VALUE) {
					actions.add(Action.createCompoundGather(lumberjackID, closestID));
				}
				
			}
		}
	}
	private void assignWorkerToReturn(StateView state, UnitView worker, Collection<Action> actions) {
		...
		for (Integer potentialStoragePitID : state.getUnitIds(playerID))
		{
			...
			if (closestID != Integer.MIN_VALUE) {
				actions.add(Action.createCompoundDeposit(worker.getID(), closestID));
			}
		}
	}
